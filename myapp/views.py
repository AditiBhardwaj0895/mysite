...
sendgrid_key="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"
...
app = ClarifaiApp(api_key='bdff7a7bd945463792ed3aafdc1f1350')
model = app.models.get('general-v1.3')

from paralleldots import set_api_key, sentiment
PKEY = 'sUhUUKUgnEdjYdhv9yxiWpRFV32ceNB7lhxkUGnf53Y'
...
def profile_view(request, username):
        user = check_validation(request)
        if user:
            user_profile = UserModel.objects.all().filter(username=username)
            print user_profile
    ...
    if user and request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if sentiment_score >= 0.6:
                ...
                post = LikeModel.objects.create(post_id=post_id, user=user)
                #sg = sendgrid.SendGridAPIClient(apikey='sendgrid_key')
                #from_email = Email("aditi.sre@gmail.com")
